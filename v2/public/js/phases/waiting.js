  ...
  root.querySelectorAll('.segmented').forEach(seg => {
    seg.querySelectorAll('.seg-btn').forEach(btn => {
      btn.onclick = () => {
        ...
        if (seg.dataset.seg === 'mult')  rules.sipsMultiplier = Number(raw);
        if (seg.dataset.seg === 'timer') rules.timerDuration  = Number(raw);
        if (seg.dataset.seg === 'oralInterval') rules.oralInterval = Number(raw);
        if (seg.dataset.seg === 'cardStyle') rules.cardStyle = raw;
      };
    });
  });
